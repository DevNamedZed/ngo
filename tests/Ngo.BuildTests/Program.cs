...
static class Analyzer
{
    ...
    public static IReadOnlyList<CompileError> AnalyzePackageDir(string dir)
    {
        ...
        catch (PackageCacheBuildException ex)
        {
            ...
            failures.Add(new CompileError(
                ...
                new TextSpan(0, 0),
                $"dependency '{ex.ImportPath}' failed cache build: {ex.Reason}"));
    ...
    private static void InjectSyntheticSources(string dir, List<SyntaxTree> trees)
    {
        ...
        if (dirName.EndsWith("/internal/buildcfg"))
        {
            trees.Add(SyntaxTree.Parse(
                "package buildcfg\n\n" +
                "import \"runtime\"\n\n" +
        ...
        if (dirName.EndsWith("/runtime/internal/sys"))
        {
