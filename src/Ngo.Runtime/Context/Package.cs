...
namespace Ngo.Runtime.Context
{
    [GoPackage("context")]
    [GoType("interface", Name = "Context", Package = "context")]
    public sealed class GoContext
    {
        ...
        [GoMethod]
        [return: GoReturn("time.Time", "bool")]
        ...
        [GoVar(Type = "error")]
        public static readonly object? Canceled = "context canceled";

        // context.DeadlineExceeded error
        [GoVar(Type = "error")]
        public static readonly object? DeadlineExceeded = "context deadline exceeded";

        // context.WithCancelCause(parent Context) (ctx Context, cancel CancelCauseFunc)
        public static (GoContext, Action<object?>) WithCancelCause(GoContext parent)
        {
            var cts = new CancellationTokenSource();
            var ctx = new GoContext(parent, cts, null, null);
            return (ctx, (cause) => cts.Cancel());
        }

        // context.Cause(c Context) error
        public static object? Cause(GoContext c)
        {
            return c.Err();
        }

        // context.AfterFunc(ctx Context, f func()) (stop func() bool)
        public static Func<bool> AfterFunc(GoContext ctx, Action f)
        {
            return () => false;
        }
